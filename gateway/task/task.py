...
def create(request, user_id):
    ...
    f"http://172.0.0.5:6000/create", data=payload, headers=headers
    )
...
def get(request, user_id):
    ...
    f"http://172.0.0.5:6000/get?user_id={user_id}"
    )
    if response.status_code == 200:
...
def remove(request, user_id, task_id):
    ...
    f"http://172.0.0.5:6000/remove/{task_id}?user_id={user_id}"
    )
